...
// var
$cprojectName = filter_var($_POST['cprojectName'], FILTER_SANITIZE_STRING);
...
$_cproject = mysqli_query($master, "
    SELECT * FROM cproject
    WHERE cprojectName = '" . $cprojectName . "'
    AND cprojectStatus = 1
");

$cproject = $_cproject -> fetch_object();

$cprojectDb = $cproject -> cprojectName;

if (strcasecmp($cprojectName, $cprojectDb) !== 0) {

    $insert = $master -> prepare ("
        INSERT INTO cproject
        (cprojectName, cprojectText, cprojectText2)
        VALUES
        (?,?,?)
    ");
