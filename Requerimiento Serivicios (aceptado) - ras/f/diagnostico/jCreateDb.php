...
// var
$rasName = filter_var($_POST['rasName'], FILTER_SANITIZE_STRING);
$rasText = filter_var($_POST['rasText'], FILTER_SANITIZE_STRING);
$rasText2 = filter_var($_POST['rasText2'], FILTER_SANITIZE_STRING);

// verify

$_ras = mysqli_query($master, "
    SELECT * FROM ras
    WHERE rasName = '" . $rasName . "'
    AND rasStatus = 1
");

$ras = $_ras -> fetch_object();

$rasDb = $ras -> rasName;

if (strcasecmp($rasName, $rasDb) !== 0) {

    $insert = $master -> prepare ("
        INSERT INTO ras
        (rasName, rasText, rasText2)
        VALUES
        (?,?,?)
    ");
    ...
    foreach ($_FILES["archivo2"]['tmp_name'] as $key => $tmp_name) {
        ...
        if ($_FILES["archivo2"]["name"][$key]) {
            ...
            $directorio2 = '../ras/' . $id . '/'; //Declaramos un  variable con la ruta donde guardaremos los archivos
